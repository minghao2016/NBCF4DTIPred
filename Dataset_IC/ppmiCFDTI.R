...
ppmiCFDTI <- function(db = "hao",
  ...
  if (db == "hao") {
    ...
    simDrug <- read.table("ic_simmat_dc.txt")
    simDrug <- as.matrix(simDrug) ## 210 drugs * 210 drugs
    
    simProt <- read.table("ic_simmat_dg.txt")
    simProt <- as.matrix(simProt) ## 204  targets * 204 targets
    
    adjMat <- read.table("ic_admat_dgc.txt")
    adjMat <- as.matrix(adjMat) ## 204 targets *  210 drugs
  }
  else {
